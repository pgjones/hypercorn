...
async def test_http_server_drain(event_loop: asyncio.AbstractEventLoop) -> None:
    ...
    server = hypercorn.asyncio.base.HTTPServer(event_loop, Config(), transport, '')  # type: ignore
...
def test_suppress_body(method: str, status: int, expected: bool) -> None:
    assert hypercorn.asyncio.base.suppress_body(method, status) is expected


def test_response_headers(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(hypercorn.asyncio.base, 'time', lambda: 1512229395)
    assert hypercorn.asyncio.base.response_headers('test') == [
